...
# db string for the pooled by fraction
        db_string_fraction='tfg_assembly.orf'
...
# making the final output name, but now saving it to a given directory
        output_file_name_dir='../data/tfg-all-database/'$idxml_file_name'_tfg_all'
...
# running the database search
        MSGFPlusAdapter -in $FILE -executable /software/MSGFPlus/MSGFPlus_Oct2018.jar -database $db_string_adjusted -out $output_file_name_dir'.idXML' -add_decoys -threads 32 -java_memory 7000
